...
```
exit 0 → delegated mode confirmed. Load the matching field artifact (object, API name, type, length/picklist values, etc.). **Idempotency check (P3)**: read canonical state.json; if this artifact id is `done` exit no-op (`idempotent: <id> already done`); if `in_progress` AskUserQuestion `[Continue / Restart / Skip]`. Otherwise skip the Step 1 question battery, start from Step 2 context analysis. On completion/failure the caller updates status via dispatch-state-cli (canonical `.harness-sf/state/<slug>__r<rev>.json`).
